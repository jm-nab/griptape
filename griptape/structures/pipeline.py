...
class Pipeline(Structure):
    _tasks: list[BaseTask] = field(factory=list, kw_only=True, alias="ordered_tasks")
    ...
    def _from_task_graph(self) -> None:
        for task, parents in self._task_graph.items():
            if len(parents) > 1:
                raise ValueError("Pipelines can only be a straight sequence of tasks.")
            self._tasks.extend([task, *parents])
        self._tasks = self.ordered_tasks
